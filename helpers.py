...
def fix_team(df, col):
    ...
    """
    df[col] = df.apply(lambda row: 'ARI' if row[col] == 'PHX' else row[col], axis=1)
    df[col] = df.apply(lambda row: 'WPG' if row[col] == 'ATL' else row[col], axis=1)

    return df


def merge_outcomes(stats_df):
    """
    Merge the outcomes from each game into the main DataFrame (the total outcomes is already saved)
    
    :param stats_df: DataFrame we want the outcomes merged into
    
    :return Given DataFrame with game outcomes merged in
    """
    outcomes_df = pd.read_csv("./data/game_outcomes.csv", index_col=0)
